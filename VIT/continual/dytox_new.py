...
class DyTox_New(nn.Module):
    """"DyTox for the win!

    :param transformer: The base transformer.
    :param nb_classes: The initial number of classes.
    ...
    def __init__(
        ...
        if self.use_resnet:
            ...
            self.patch_embed_mask = transformer.patch_embed
            self.pos_embed_mask = transformer.pos_embed
            self.pos_drop_mask = transformer.pos_drop
            self.sabs_mask = transformer.masks_net
    ...
    def mask_net(self, dim):

        layers = nn.Sequential(nn.Linear(dim, dim),
                  nn.LayerNorm(dim),
                  nn.ReLU(inplace=True),
                  nn.Linear(dim, dim),
                  nn.LayerNorm(dim),
    ...
    def cnn_net(self, in_maps, out_maps):
    ...
    def forward_features(self, x):
        ...
        if self.use_resnet:
            ...
            # x_mask = self.patch_embed_mask(x)
            # x_mask = x_mask + self.pos_embed_mask
            # x_mask = self.pos_drop_mask(x_mask)
            # self.masks = []
            # for i in range(len(self.sabs_mask)):
            #     x_mask = self.sabs_mask[i](x_mask)
            #     self.masks.append(x_mask)

            masks_list = self.sabs_mask(x)

            x = self.patch_embed(x)
            x = x + self.pos_embed
            x = self.pos_drop(x)
            self.feats = []
            for i in range(len(self.sabs)):
                x, attn, v = self.sabs[i](x)
                # if i<len(self.sabs)-4:
                x = masks_list[i]*x
                self.feats.append(x)
