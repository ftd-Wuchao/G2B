...
class Cifar_Mask_net(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768 , masks_num=5):
        super().__init__()
        ...
        self.masks_num = masks_num

        self.pre_net = self.pre_embeding()
        self.masks = nn.ModuleList([self.mask_net() for _ in range(masks_num)])
        self.proj_heads = nn.ModuleList([self.proj_head() for _ in range(masks_num)])
        #
        # self.masks = nn.ModuleList([self.mask_net(128*(2^i)) for i in range(masks_num)])
        # self.proj_heads = nn.ModuleList([self.proj_head(256*(2^i)) for i in range(masks_num)])
    ...
    def pre_embeding(self):
        layers = [nn.Conv2d(3, 128, kernel_size=3, stride=1, padding=1),
                  nn.BatchNorm2d(128),
                  nn.ReLU(inplace=True),
                  nn.Conv2d(128, self.embed_dim, kernel_size=3, stride=1, padding=1),
    ...
    def mask_net(self): #
        layers = [
                  nn.Conv2d(self.embed_dim, int(self.embed_dim/2), kernel_size=3, stride=1, padding=1),
                  nn.BatchNorm2d(int(self.embed_dim/2)),
                  nn.ReLU(inplace=True),
                  nn.Conv2d(int(self.embed_dim/2), self.embed_dim,  kernel_size=3, stride=1, padding=1),
                  nn.BatchNorm2d(self.embed_dim),
                  nn.ReLU(inplace=True),
                  ]
        # layers = [
    ...
    def proj_head(self): #
        layers = [
                  nn.Conv2d(self.embed_dim, self.embed_dim, kernel_size=self.patch_size , stride=self.patch_size ),
                  nn.BatchNorm2d(self.embed_dim),
                  nn.ReLU(inplace=True),
                  ]
        # layers = [
        #           nn.Conv2d(input_dim, self.embed_dim, kernel_size=self.patch_size, stride=self.patch_size),
        #           nn.BatchNorm2d(self.embed_dim),
        #           nn.ReLU(inplace=True),
        #           ]
        return nn.Sequential(*layers)
    ...
    def forward(self, x):
        x = self.pre_net(x)
        masks_list = []
        for i in range(self.masks_num):
            x = self.masks[i](x)
            masks_list.append(self.proj_heads[i](x).flatten(2).transpose(1, 2))
        # x = self.pre_net(x)
        # masks_list = []
        # for i in range(self.masks_num):
        #     x = self.masks[i](x)
        #     x_mask = self.proj_heads[i](x)
        #     x_mask_ = torch.mean(x_mask, dim=1, keepdim=True)
        #     x_mask_ = self.relu(x_mask_.expand_as(x_mask))
        #     masks_list.append( x_mask_.flatten(2).transpose(1, 2))
        return masks_list


class Imagenet_Mask_net(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768 , masks_num=5):
        super().__init__()
    ...
    def pre_embeding(self):
        layers = [nn.Conv2d(3, 128, kernel_size=7, stride=2, padding=3),
                  nn.BatchNorm2d(128),
                  nn.ReLU(inplace=True),
    ...
    def proj_head(self): #
        layers = [
                  nn.Conv2d(self.embed_dim, self.embed_dim, kernel_size=int(self.patch_size/4) , stride=int(self.patch_size/4)),
                  nn.BatchNorm2d(self.embed_dim),
                  nn.ReLU(inplace=True),
                  ]
        return nn.Sequential(*layers)
...
class ConVit(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if img_size==32:
            self.masks_net = Cifar_Mask_net(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim , masks_num=local_up_to_layer).cuda()
        elif img_size==224:
            self.masks_net = Imagenet_Mask_net(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim , masks_num=local_up_to_layer).cuda()
        else:
            pass
