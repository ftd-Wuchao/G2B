...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        # self.cnn = nn.Sequential(nn.Conv2d(512, 512, kernel_size=5, stride=1,
        ...
        # self.fc = nn.Sequential(nn.Linear(512 * block.expansion, 512 * block.expansion),
        ...
        # self.f_conv1 = self._make_conv2d_layer(3, nf, max_pool=False,stride=1, padding=1)
        # self.f_conv2 = self._make_conv2d_layer(nf * 1, nf * 2, padding=1, stride=1, max_pool=True)
        # self.f_conv3 = self._make_conv2d_layer(nf * 2, nf * 4, padding=1, stride=1, max_pool=True)
        # self.f_conv4 = self._make_conv2d_layer(nf * 4, nf * 8, padding=1, stride=1, max_pool=True)

        # self.f_conv = Mask_Net(3, nf, max_pool=False, stride=1, padding=1, start = True)
        self.f_conv1 = self._make_conv2d_layer(3, nf, kernel_size=7, stride=2, padding=3, max_pool=True)
        self.f_conv2 = self._make_conv2d_layer(nf * 1, nf * 2, padding=1, stride=1, max_pool=True)
        self.f_conv3 = self._make_conv2d_layer(nf * 2, nf * 4, padding=1, stride=1, max_pool=True)
        self.f_conv4 = self._make_conv2d_layer(nf * 4, nf * 8, padding=1, stride=1, max_pool=True)
    ...
    @staticmethod
    def _make_conv2d_layer(in_maps, out_maps, kernel_size=3, max_pool=False, stride=1, padding=1):
        layers = [nn.Conv2d(in_maps, out_maps, kernel_size=kernel_size, stride=stride, padding=padding),
                  nn.BatchNorm2d(out_maps),
                  nn.ReLU(inplace=True),
                  nn.Conv2d(out_maps, out_maps, kernel_size=3, stride=1, padding=1),
                  nn.BatchNorm2d(out_maps),
                  nn.ReLU(inplace=True)
                  ]
        if max_pool:
    ...
    def _forward_impl(self, x):
        ...
        # h1,h2,h3,h4 = self.attention_net(x)
        m0 = self.conv1(x)  # [bs, 64, 32, 32]
        m0 = self.bn1(m0)
        m0 = self.relu(m0)
        m0 = self.maxpool(m0)
...
class Mask_Net(nn.Module):
    def __init__(self, in_maps, out_maps, kernel_size=3, max_pool=False, stride=1, padding=1):
    ...
    def forward(self, x):
        ...
        neg_mask = mask_bf.detach() < 0
        mask = self.relu(mask_bf)
        if self.max_pool:
            mask = self.max_pool_layer(mask)
            mask_bf = self.max_pool_layer(mask_bf)
            neg_mask = mask_bf.detach()  < 0
        return mask, mask_bf, neg_mask
...
class attention_net(nn.Module):
    ...
    def forward(self, x):
        h1 = self.convs_forward(self.f_conv1_list, x)
        h2 = self.convs_forward(self.f_conv2_list, h1)
        h3 = self.convs_forward(self.f_conv3_list, h2)
        h4 = self.convs_forward(self.f_conv4_list, h3)
        return h1, h2, h3, h4

    def convs_forward(self, convs_list, x):
        h = 0
        for i, f_conv1 in enumerate(convs_list):
            if i != self.size:
                h += f_conv1(x).detach()
            else:
                h += f_conv1(x)
