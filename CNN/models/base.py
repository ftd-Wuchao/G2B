...
class BaseLearner(object):
    ...
    def _extract_vectors(self, loader):
        ...
        for _, _inputs, _targets in loader:
            ...
            if self.args['ssl_train']:
                # print(_inputs.size())
                if _inputs.size()[1] != 3 :
                    _inputs = self.transformer_fn(_inputs)
                if isinstance(self._network, nn.DataParallel):
    ...
    def _caculate_prototype(self, data_manager):
        if self._cur_task==0:
            self._class_means = np.zeros((self._total_classes, self.feature_dim))
        else:
            temp = np.zeros((self._total_classes-self._known_classes, self.feature_dim))
            self._class_means = np.concatenate((self._class_means, temp), axis=0)
        ...
        for class_idx in range(self._known_classes,  self._total_classes):
            ...
            # train_class_mean.append(torch.tensor(np.mean(vectors, axis=1)).to(self._device))
            # covariance_matrix.append(torch.tensor(np.cov(vectors.T)).to(self._device))
            vectors = (vectors.T / (np.linalg.norm(vectors.T, axis=0) + EPSILON)).T
            mean = np.mean(vectors, axis=0)
            mean = mean / np.linalg.norm(mean)
            self._class_means[class_idx, :] = mean

        # self._train_class_mean.append(train_class_mean)
        # self._covariance_matrix.append(covariance_matrix)
