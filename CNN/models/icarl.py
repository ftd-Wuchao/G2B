...
from utils.inc_net import IncrementalNet, Muti_Mask_Branch
...
class iCaRL(BaseLearner):
    ...
    def __init__(self, args):
        ...
        # self._network = Muti_Mask_Branch(args['convnet_type'], False)
    ...
    def incremental_train(self, data_manager):
        ...
        # self._network.convnet.mask_net.append_network()
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        prog_bar = tqdm(range(epochs)) #epochs
        for _, epoch in enumerate(prog_bar):
            self._network.train()
            if self._cur_task > freeze_task:
                self._network.freeze_mask()
            losses = 0.
            correct, total = 0, 0
            for i, (_, inputs, targets) in enumerate(train_loader):
                inputs, targets = inputs.to(self._device), targets.to(self._device)
                ...
                loss=loss_clf+kd_lamda*loss_kd
