...
class CenterLoss(nn.Module):
    def __init__(self, an_margin=0.7, ap_margin= 0.3):
    ...
    def forward(self, inputs, targets):
        ...
        targets_ = torch.unique(targets).cpu()
        # num_class = len(targets_)
        num_class = targets_.size(0)

        targets_ = torch.LongTensor(targets_).to(targets.device)
